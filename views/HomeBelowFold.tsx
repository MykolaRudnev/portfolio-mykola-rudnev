  ...
  return (
    <section className="px-6 py-14 border-t border-rule">
      <div className={PAGE_CONTENT_CLASS}>
        ...
        <p className="text-ink-2 text-center text-sm max-w-2xl mx-auto mb-10">
          ...
          <PrefetchLink
            ...
            className="text-accent underline underline-offset-4 hover:text-accent"
          >
            case studies
          </PrefetchLink>
          .
...
function TrustStrip() {
  return (
    <section className="px-6 py-10 border-y border-rule">
      <div className="max-w-5xl mx-auto">
        <p className="text-center text-sm text-ink-2 mb-5 uppercase tracking-widest">Core stack</p>
        <div className="flex flex-wrap justify-center gap-2">
          {STACK_BADGES.map((badge) => (
            <span
              key={badge}
              className="px-4 py-2 rounded-full bg-paper-2 border border-rule text-sm text-ink-2"
            >
              {badge}
            </span>
          ))}
  ...
  return (
    <section className="px-6 py-8">
      <GlassCard className="max-w-3xl mx-auto p-8">
        <h2 className="text-xl font-bold mb-4 text-accent">Current availability</h2>
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          ...
          <PrefetchLink
            ...
            className="text-sm text-accent underline underline-offset-4 hover:text-accent shrink-0 transition-colors"
          >
            Full experience →
          </PrefetchLink>
        </div>
  ...
  return (
    <li className="flex gap-5 items-stretch">
      <div className="flex w-12 shrink-0 flex-col items-center pt-0.5">
        <div
          className={`relative z-10 flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl border bg-sheet ${
            isCurrent ? "border-accent/40 " : "border-rule"
          }`}
        >
          <img
            src={experience.logo}
          ...
          />
          {isCurrent && (
            <span className="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-accent ring-2 ring-paper" />
          )}
        </div>
        {!isLast && (
          <span
            className="my-2 w-px min-h-8 flex-1 bg-rule"
            aria-hidden
          />
        )}
      </div>
      ...
      <article className={`min-w-0 flex-1 ${isLast ? "pb-0" : "pb-10"}`}>
        <h3 className="font-semibold text-ink leading-snug">{experience.position}</h3>
        <p className="mt-0.5 text-sm font-medium text-accent">{experience.company}</p>
        <p className="mt-2 inline-flex items-center rounded-full border border-rule bg-paper-2 px-2.5 py-0.5 text-xs text-ink-2">
          {experience.period}
        </p>
      </article>
    </li>
